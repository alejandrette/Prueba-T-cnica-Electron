...
async function saveImage (mainWindow) {
  try {
    ...
    for (const img of images) {
      ...
      logger.info(`Inicio de descarga: ${filename}`) // Log de inicio de descarga
      ...
      try {
        ...
        await new Promise((resolve, reject) => {
          writer.on('finish', () => {
            ...
            mainWindow.webContents.send('nueva-imagen', fullPath) // Enviar la ruta de la imagen al renderer
            mainWindow.webContents.send('log-desde-main', `Imagen guardada: ${filename}`) // Enviar mensaje al renderer
          ...
          writer.on('error', (err) => {
            logger.error(`Error al guardar la imagen: ${filename} - ${err.message}`)
            mainWindow.webContents.send('log-desde-main', `Error guardando ${filename}: ${err.message}`)
            reject(err)
          })
        })
      } catch (error) {
        // Manejar errores al descargar la imagen
        mainWindow.webContents.send('log-desde-main', `Error descargando ${filename}: ${error.message}`)
        logger.error(`Error descargando ${filename}: ${error.message}`)
      }
    }
  } catch (error) {
    // Manejar errores al leer el archivo JSON
    mainWindow.webContents.send('log-desde-main', `Error general: ${error.message}`)
